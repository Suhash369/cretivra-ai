  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#060911] overflow-hidden p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-[#E7EAF4] tracking-tight">Projects</h1>
        ...
        {isLoading ? (
          ...
          filtered.map((p) => (
            <div
              ...
              className="p-5 rounded-2xl bg-[#0D121F] border border-[#232D45] hover:border-[#06B6D4]/40 transition-all cursor-pointer flex flex-col justify-between group asura-card-interactive shadow-xs"
            >
              <div>
                <div className="w-10 h-10 rounded-xl bg-[#151C2E] border border-[#232D45] flex items-center justify-center text-[#06B6D4] mb-3">
                  <FolderGit2 size={18} />
                </div>
                ...
                {p.description && (
                  <p className="text-xs text-[#8891A8] mt-1 line-clamp-2 leading-relaxed">
                    {p.description}
                  </p>
                )}
              </div>

              <div className="mt-4 pt-3 border-t border-[#232D45]/60 flex items-center justify-between text-[11px] text-[#8891A8]">
                <span>
                  {p.updated_at ? `Updated ${new Date(p.updated_at).toLocaleDateString()}` : 'Active Sandbox'}
                </span>
                <ArrowRight size={13} className="text-[#8891A8] group-hover:text-[#06B6D4] transition-colors" />
              </div>
            </div>
          ))
        )}
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-[#0D121F] border border-[#232D45] rounded-2xl p-5 shadow-2xl animate-scale">
            <h2 className="text-base font-semibold text-[#E7EAF4]">Create New Project</h2>
            ...
            <div className="mt-4 space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-[#151C2E] border border-[#232D45] rounded-xl text-xs text-[#E7EAF4] focus:outline-none focus:border-[#06B6D4]/50"
                  autoFocus
                />
              </div>
