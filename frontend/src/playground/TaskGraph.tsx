    ...
    <div className="flex flex-col h-full bg-[#0d121f] border border-[#232d45] rounded-xl overflow-hidden">
      <div className="px-4 py-3 border-b border-[#232d45] flex items-center justify-between bg-[#151c2e]/50">
        ...
        <div className="flex items-center gap-3 text-[10px] text-slate-400">
          ...
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-cyan-400 inline-block" /> Running</span>
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-emerald-400 inline-block" /> Done</span>
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" /> Approval</span>
        </div>
      </div>
        ...
        {tasks.map((task, idx) => {
          ...
          const nodeColorClass = (() => {
            switch (task.status) {
              ...
              case 'RUNNING':
                return 'border-cyan-500/80 bg-cyan-950/30 text-cyan-300 ring-1 ring-cyan-500/50';
              case 'WAITING_FOR_APPROVAL':
                return 'border-amber-500/80 bg-amber-950/30 text-amber-300 ring-1 ring-amber-500/50';
              case 'FAILED':
                return 'border-rose-500/60 bg-rose-950/20 text-rose-300';
              default:
                return 'border-slate-800 bg-[#151c2e]/60 text-slate-400';
            }
          })();
