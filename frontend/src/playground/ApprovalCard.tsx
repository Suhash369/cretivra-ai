...
export function ApprovalCard({ approval, onApprove, onDeny, isProcessing }: ApprovalCardProps) {
  ...
  return (
    <div className="bg-[#151c2e] border-2 border-amber-500/60 rounded-xl p-4 shadow-2xl shadow-amber-950/40 animate-in fade-in zoom-in-95">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <h4 className="text-sm font-semibold text-slate-100 mt-1">{approval.description}</h4>
          <p className="text-xs text-slate-400 mt-0.5">
            Tool: <code className="text-cyan-400 font-mono">{approval.tool_name}</code>
          </p>
            ...
            <div className="mt-3">
              {isEditing ? (
                <textarea
                  ...
                  className="w-full h-24 p-2 text-xs font-mono bg-slate-950 border border-slate-700 rounded-lg text-slate-200 focus:outline-none focus:border-amber-400"
                />
              ) : (
                <pre className="p-2.5 rounded-lg bg-slate-950/80 border border-slate-800 text-[11px] font-mono text-slate-300 overflow-x-auto max-h-32">
          ...
          <div className="flex items-center gap-2 mt-4">
            ...
            <button
              ...
              disabled={isProcessing}
              className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-rose-600/20 hover:bg-rose-600/30 text-rose-300 border border-rose-500/30 text-xs font-semibold transition-colors disabled:opacity-50 cursor-pointer"
            >
