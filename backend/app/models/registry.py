...
class CretivraModelRegistry:
    ...
    def is_image_model(self, model_id: str) -> bool:
        ...
        if not model:
            return False
        return "image" in model.capabilities or model.category == "Image Studio" or model.provider in ("pollinations", "vision_studio", "image")
    ...
    def update_availability_from_ollama_tags(self, installed_tags: List[str], mock_mode: bool = False):
        ...
        for m in self._models.values():
            if m.provider in ("pollinations", "vision_studio", "cloud", "frontier_core", "image") or "image" in m.capabilities or "vision" in m.capabilities:
                m.is_available = True
            elif mock_mode:
                m.is_available = True
            else:
